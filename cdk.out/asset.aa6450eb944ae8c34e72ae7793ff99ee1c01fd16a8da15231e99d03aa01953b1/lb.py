...
MONGODB_URI = os.environ.get("MONGODB_URI")  # CDK writes this into config.py on EC2
if not MONGODB_URI:
    # fallback: try to import generated config (CDK user-data should create it)
    try:
        from scripts.config import MONGODB_URI as MONGODB_URI2
...
class ConsistentHashRing:
    def __init__(self, vnodes_per_weight=10):
        self.ring = {}           # hash -> server_doc
        self.sorted_keys = []    # sorted hash list
    ...
    def build(self):
        ...
        for s in servers:
            ...
            for i in range(vcount):
                ...
                # in case of hash collision, just overwrite - acceptable for demo
                self.ring[h] = s
...
def route_key(key):
    ...
    try:
        ...
        # increment load_count in DB
        servers_col.update_one({"_id": server["_id"]}, {"$inc": {"load_count": 1}})
        broadcast_state()
        return jsonify(resp.json())
    except Exception as e:
        # mark server unhealthy if unreachable
